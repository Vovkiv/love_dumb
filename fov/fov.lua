...
module.march_line = function (map, view, c1, r1, c2, r2, radius) -- source and target
	local length = module.abs(c2 - c1) + module.abs(r2 - r1)
	if length > 1 then
		local dx = (c2 - c1) / length
		local dy = (r2 - r1) / length
		for n = 1, length do
			local c, r = module.floor (c1 + n * dx + 0.5 + 0.00001 * dy), module.floor (r1 + n * dy + 0.5 - 0.00001 * dx)

      if radius^2 <= (c1 - c)^2 + (r1 - r)^2 then
...
module.get = function (map, c, r, radius)
	...
	for c2 = c - radius, c + radius do
		module.march_line (map, view, c, r, c2, r + radius, radius)
		module.march_line (map, view, c, r, c2, r - radius, radius)
	end
	for r2 = r - radius + 1, c + radius - 1 do
		module.march_line (map, view, c, r, c + radius, r2, radius)
		module.march_line (map, view, c, r, c - radius, r2, radius)
	end
	return view
end
